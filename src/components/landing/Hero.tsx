...
export function Hero() {
  ...
  return (
    <section className="relative max-w-6xl mx-auto pt-8 pb-24 px-6 text-center">
      ...
      >
        <Zap size={12} fill="currentColor" /> {t('marketingTag')}
      </motion.div>
      ...
      >
        {t.rich('heroTitle', {
          ops: (chunks) => <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-fuchsia-500 to-amber-400 animate-gradient-x">{chunks}</span>
        })}
      </motion.h1>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="max-w-3xl mx-auto">
        <h2 className="text-2xl md:text-4xl font-bold text-slate-800 mb-8 tracking-tight leading-tight">
          {t.rich('heroSubtitle', {
            dynamicMinds: (chunks) => <span className="text-indigo-600">{chunks}</span>
          })}
        </h2>

        <p className="text-lg md:text-xl text-slate-500 mb-12 leading-relaxed max-w-2xl mx-auto font-medium">
          {t.rich('heroDescription', {
            operationalLayer: (chunks) => <span className="text-slate-900 font-bold">{chunks}</span>
          })}
        </p>
        ...
        >
          {t('activateProtocol')} <ArrowRight size={22} />
        </button>
